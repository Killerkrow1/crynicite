...
public class PyriteChunk extends Item {
    public PyriteChunk(Settings settings) {
        super(settings);
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (!world.isClient()) {
            ThrownItemEntity throwableEntity = new PyriteChunkEntity(world, user.getX(), user.getY()+1.5 - 0.1D, user.getZ());
